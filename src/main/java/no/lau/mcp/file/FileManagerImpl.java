...
public class FileManagerImpl implements FileManager {
     ...
     */
    @Override
    public Map<String, Path> listFilesWithGeneratedKeys() throws IOException {
     ...
     *
     * @param extension The desired file extension (e.g., ".txt", ".pdf"). Should start with a dot.
     * @return The Path to the newly created file (absolute and normalized).
     * @throws IOException if an I/O error occurs.
     * @throws IllegalArgumentException if the extension is null, empty, or invalid.
     * @throws SecurityException if path resolution leads outside the destination.
     */
    public Path createNewFileWithAutoGeneratedNameInSecondFolder(String extension) throws IOException {
        if (extension == null || extension.trim().isEmpty() || !extension.startsWith(".")) {
            throw new IllegalArgumentException("Extension must be provided and start with a dot (e.g., '.txt').");
        }
        // Basic sanitization for the extension to prevent malicious characters.
        // Allowing alphanumeric, dot, and hyphen. Adjust regex as needed for more complex valid extensions.
        if (!extension.matches("^[.a-zA-Z0-9_-]+$")) {
            throw new IllegalArgumentException("Extension contains invalid characters: " + extension);
        }
        ...
        if (!absoluteNormalizedNewFilePath.startsWith(this.destinationFolder)) {
            ...
            System.err.println("Destination Folder: " + this.destinationFolder);
            System.err.println("Generated File Name with custom extension: " + generatedFileName);
            System.err.println("Attempted Normalized Path: " + absoluteNormalizedNewFilePath);
            throw new SecurityException("Attempted to create file outside of the designated destination folder.");
        }

        log.info("Securely creating new file with auto-generated name and custom extension '" + extension + "' at: " + absoluteNormalizedNewFilePath);
        return Files.createFile(absoluteNormalizedNewFilePath);
    }
    ...
    public Set<String> listVideoReferences() {
        try {
            ...
            //TODO don't list files from disk all the time, read them on startup or face latency issues!
    ...
    /**
     * Generates a temporary file with a default prefix ("temp_") and suffix (".tmp").
     * The file is registered to be automatically deleted when the Java Virtual Machine terminates.
     *
     * @return The Path to the newly created temporary file.
     * @throws IOException if an I/O error occurs or if a file could not be created.
    ...
    public Path createTemporaryFile() throws IOException {
        return createTemporaryFile("tempFile_", ".tmp");
    }

    public String replaceVideoReferences(String command)  {
        try {
            Map<String, Path> videoReferences = listFilesWithGeneratedKeys();
            return FileManager.replaceVideoReferences(command, videoReferences);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
